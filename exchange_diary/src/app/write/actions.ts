...
function stateForResult(result: MatchResult): SubmissionState {
...
function submitDemoEntry({
  ...
  return stateForResult({ status: "matched", exchange_id: exchangeId });
}
...
export async function submitDiaryEntry(
  ...
  try {
    ...
    return stateForResult(data as MatchResult);
  } catch {
    return {
      status: "error",
